                      ...
                      {isEditing && (
                        <div className="absolute -right-2 -top-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                          <button
                            ...
                            disabled={idx === 0}
                            className="bg-white border border-gray-200 rounded-full w-6 h-6 flex items-center justify-center hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
                            title="Move Up"
                          >
                            ↑
                          </button>
                          <button
                            onClick={() => {
                              if (!projects || idx === projects.length - 1)
                                return;
                              const newProj = [...projects];
                              [newProj[idx], newProj[idx + 1]] = [
                                newProj[idx + 1],
                                newProj[idx],
                              ];
                              updateField("projects", newProj);
                            }}
                            disabled={!projects || idx === projects.length - 1}
                            className="bg-white border border-gray-200 rounded-full w-6 h-6 flex items-center justify-center hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
                            title="Move Down"
                          >
                            ↓
